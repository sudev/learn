...
object TimeUsage {
  ...
  def dfSchema(columnNames: List[String]): StructType =
    StructType(StructField(columnNames.head, StringType, false) :: columnNames.tail.map(x => StructField(x, DoubleType, false)))
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    val createColumn = (listColNames: List[String]) => listColNames.map(colName => new Column(colName))
    ...
    (createColumn(primaryNeeds), createColumn(workingActivities), createColumn(otherActivities))
  }
